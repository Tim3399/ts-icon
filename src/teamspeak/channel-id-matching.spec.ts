...
describe("matchChannelIdsToRows", () => {
  it("matches a row to a live channel whose normalized name equals the row channelName", () => {
    ...
    expect(result.matched).toEqual([{ channelName: "general", channelId: "1" }]);
  ...
  it("handles a mix of matched and unmatched rows in the same run", () => {
    const rows = [{ channelName: "general" }, { channelName: "gone" }, { channelName: "music" }];
    const liveChannels: LiveChannel[] = [
      { cid: "1", name: "General", bannerGfxUrl: null, pid: null },
  ...
  it("reports ambiguous names without assigning either channel ID", () => {
    // A normalization collision must be reviewed instead of assigning
    // whichever live channel happens to appear first or last.
    const rows = [{ channelName: "rohre" }];
    const liveChannels: LiveChannel[] = [
    ...
    expect(result.matched).toEqual([]);
    expect(result.conflicts).toEqual([{ channelName: "rohre", channelIds: ["1", "2"] }]);
  });
});
