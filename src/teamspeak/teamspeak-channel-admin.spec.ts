...
let channels: TeamSpeakChannelsService;

beforeEach(() => {
  channels = new TeamSpeakChannelsService();
});
...
describe("createManagedChannel", () => {
  it("creates a permanent channel with the banner URL set at creation time", async () => {
    const channelCreate = jest.fn().mockResolvedValue(fakeChannel("10", "Art 1"));
...
describe("deleteManagedChannels", () => {
  it("deletes every given cid over a single connection and invalidates the cache", async () => {
    ...
    const result = await deleteManagedChannels(channels, ["1", "2", "3"]);

    expect(mockedConnect).toHaveBeenCalledTimes(1);
    expect(channelDelete).toHaveBeenCalledTimes(3);
    expect(channelDelete).toHaveBeenNthCalledWith(1, "1", false);
  ...
  it("invalidates the live-channels cache once finished", async () => {
    ...
    await deleteManagedChannels(channels, ["1"]);
...
describe("createChannelWallpaper", () => {
  function slice(name: string, depth: number, isSpacer: boolean): WallpaperChannelSlice {
    return {
      name,
      depth,
      isSpacer,
  ...
  it("creates every slice in order, chaining sibling order at each depth (flat mode)", async () => {
    ...
    const slices = [slice("Art1", 0, false), slice("Spacer1", 0, true), slice("Art2", 0, false)];
    const result = await createChannelWallpaper(fakeTeamSpeak as never, "5", slices);

    expect(result.created.map((c) => c.name)).toEqual(["Art1", "Spacer1", "Art2"]);
  ...
  it("stops on a mid-batch failure and reports which rows succeeded/failed, without rollback", async () => {
    ...
    const slices = [slice("Art1", 0, false), slice("Art2", 0, false), slice("Art3", 0, false)];
    const result = await createChannelWallpaper(fakeTeamSpeak as never, null, slices);
