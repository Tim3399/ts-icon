...
import { PrivateNetworkGuard, isPrivateOrLoopbackAddress } from "./private-network.guard";

function createContext(ip: string | undefined, headers = {}, ips: string[] = []): ExecutionContext {
  const request = { ip, headers, ips } as unknown as Request;
  return {
...
describe("PrivateNetworkGuard", () => {
  it("rejects forwarded headers arriving through an untrusted proxy", () => {
    const guard = new PrivateNetworkGuard();
    expect(() =>
      guard.canActivate(createContext("127.0.0.1", { "x-forwarded-for": "8.8.8.8" })),
    ).toThrow(ForbiddenException);
  });

  it("evaluates the resolved client address for an explicitly trusted proxy", () => {
    const guard = new PrivateNetworkGuard();
    expect(() =>
      guard.canActivate(createContext("8.8.8.8", { "x-forwarded-for": "8.8.8.8" }, ["8.8.8.8"])),
    ).toThrow(ForbiddenException);
    expect(
      guard.canActivate(createContext("10.0.0.5", { "x-forwarded-for": "10.0.0.5" }, ["10.0.0.5"])),
    ).toBe(true);
  });
