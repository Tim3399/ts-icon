...
describe("JwtAuthGuard", () => {
  ...
  function createGuard(metrics: MetricsService = createMetricsService()): JwtAuthGuard {
    return new JwtAuthGuard(createReflectorStub(false), localJwks, oidcConfig, metrics);
  }
  ...
  it.each(["ID", null])(
    "rejects a token without the Keycloak access token type: %s",
    async (tokenType) => {
      const token = await signToken({ tokenType });
      const { context } = createContext(`Bearer ${token}`);
      await expect(createGuard().canActivate(context)).rejects.toThrow(UnauthorizedException);
    },
  );
