 ...
 *    source address isn't loopback, so a misconfigured host doesn't
 *    silently become an open admin API.
...
export class NoAuthGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (
      !isLoopbackAddress(callerAddress) ||
      request.headers?.["x-forwarded-for"] ||
      request.headers?.forwarded
    ) {
      this.logger.warn("Rejected non-loopback request while AUTH_DISABLED=true");
