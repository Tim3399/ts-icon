...
function getMethod(ctor: { prototype: object }, name: string): object {
  const value: unknown = Object.getOwnPropertyDescriptor(ctor.prototype, name)?.value;
...
describe("@Roles()", () => {
  it("attaches the given roles as metadata on a decorated method", () => {
    ...
    const metadata: unknown = Reflect.getMetadata(ROLES_KEY, getMethod(TestController, "handler"));
