import { isBlockedIp, assertSafeUrlShape, SsrfValidationError } from "./ssrf-guard";
...
describe("assertSafeUrlShape", () => {
  it("accepts a plain https URL with no explicit port", () => {
    expect(() => assertSafeUrlShape("https://example.com/image.png")).not.toThrow();
  });

  it("accepts an explicit default HTTPS port", () => {
    expect(() => assertSafeUrlShape("https://example.com:443/image.png")).not.toThrow();
  });

  it("rejects non-https schemes", () => {
    expect(() => assertSafeUrlShape("http://example.com/image.png")).toThrow(SsrfValidationError);
    expect(() => assertSafeUrlShape("file:///etc/passwd")).toThrow(SsrfValidationError);
    expect(() => assertSafeUrlShape("ftp://example.com/image.png")).toThrow(SsrfValidationError);
  });

  it("rejects URLs with embedded credentials", () => {
    expect(() => assertSafeUrlShape("https://user:pass@example.com/image.png")).toThrow(
      SsrfValidationError,
    );
  });

  it("rejects non-default ports", () => {
    expect(() => assertSafeUrlShape("https://example.com:8443/image.png")).toThrow(
      SsrfValidationError,
    );
  });
