...
import { AUDIT_ACTION_KEY, AuditLoggingInterceptor } from "./audit-logging.interceptor";
...
describe("AuditLoggingInterceptor", () => {
  ...
  it("does not log anything when the underlying handler throws", async () => {
    ...
    const handler = createHandler(throwError(() => new Error("rejected before success")));

    await expect(firstValueFrom(interceptor.intercept(context, handler))).rejects.toThrow(
      "rejected before success",
    );
