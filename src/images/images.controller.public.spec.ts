...
describe("Public images app rate limiting (e2e)", () => {
  ...
  const mockPrismaService = {
    channelImageAlias: { findMany: jest.fn().mockResolvedValue([]) },
    channelImage: {
  ...
  it("returns 429 with a name-suffixed Retry-After header once the burst limit is exceeded", async () => {
    ...
    const blocked = await request(server).get("/images/some-channel").expect(429);
...
describe("Public images app conditional GET (ETag / If-None-Match)", () => {
  ...
  const mockPrismaService = {
    channelImageAlias: { findMany: jest.fn().mockResolvedValue([]) },
    channelImage: {
      findMany: jest.fn().mockResolvedValue([{ id: "some-channel", channelName: "some-channel" }]),
    ...
    return {
      ...
      channelImage: {
        findMany: jest
          .fn()
          .mockResolvedValue(
            Object.keys(rowsByChannelName).map((name) => ({ id: name, channelName: name })),
          ),
        findUnique: jest.fn(({ where: { id } }: { where: { id: string } }) =>
          Promise.resolve(rowsByChannelName[id] ?? null),
        ),
      },
    };
  }
