...
// SQLite database. Prisma is not mocked: this exercises `PrismaService` (real
...
describe("ImagesService (integration: real SQLite + real Prisma migrations)", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const tables = verifyDb
        .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'ChannelImage'")
        .all();
  ...
  it("saves a new row with a channelId when one is given", async () => {
    await service.saveImage("with-id", Buffer.from("v1"), "image/png", "ts-cid-1");
  ...
  it("saving again with the same channelId but a changed channelName updates the same row (rename), not a second one", async () => {
    await service.saveImage("old-name", Buffer.from("v1"), "image/png", "ts-cid-rename");
    await service.saveImage("new-name", Buffer.from("v2"), "image/jpeg", "ts-cid-rename");
    ...
    expect(Buffer.compare(renamed?.image ?? Buffer.alloc(0), Buffer.from("v2"))).toBe(0);

    const stale = await service.getImage("old-name");
    expect(stale?.image).toEqual(Buffer.from("v2"));
  ...
  it("persists size, content hash, createdAt, updatedAt, and lastEditorSubject on save", async () => {
    ...
    await service.saveImage("metadata-channel", buffer, "image/png", undefined, "editor-sub-123");

    const row = await prisma.channelImage.findFirst({
      where: { channelName: "metadata-channel" },
    });
    expect(row).not.toBeNull();
    expect(row?.size).toBe(buffer.length);
    expect(row?.contentHash).toBe(crypto.createHash("sha256").update(buffer).digest("hex"));
  ...
  it("updates updatedAt on a re-save while createdAt stays the same", async () => {
    await service.saveImage("resave-metadata-channel", Buffer.from("v1"), "image/png");
    const first = await prisma.channelImage.findFirst({
    ...
    await service.saveImage("resave-metadata-channel", Buffer.from("v2-is-longer"), "image/png");
    const second = await prisma.channelImage.findFirst({
      where: { channelName: "resave-metadata-channel" },
    });
  ...
  it("leaves an existing lastEditorSubject untouched when a later save omits it", async () => {
    ...
    await service.saveImage("editor-preserved-channel", Buffer.from("v2"), "image/png");
  ...
  it("computes a content hash on the fly for a legacy row with the empty placeholder hash, without persisting it", async () => {
    ...
    const rowAfterRead = await prisma.channelImage.findFirst({
  ...
  it("deletes an existing image and reports it was deleted", async () => {
    await service.saveImage("delete-me", Buffer.from("bytes-to-delete"), "image/png");
