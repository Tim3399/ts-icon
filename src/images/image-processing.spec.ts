...
describe("processImageForStorage", () => {
  ...
  it("rejects a corrupt/non-image buffer with InvalidImageError", async () => {
    ...
    await expect(processImageForStorage(notAnImage)).rejects.toBeInstanceOf(InvalidImageError);
  });

  it("rejects an empty buffer with InvalidImageError", async () => {
    await expect(processImageForStorage(Buffer.alloc(0))).rejects.toBeInstanceOf(InvalidImageError);
  });
  ...
  it("rejects an image whose single dimension exceeds the max before a full decode", async () => {
    ...
    await expect(processImageForStorage(input)).rejects.toBeInstanceOf(ImageTooLargeError);
  });
  ...
  it("rejects an unrecognized/unsupported format even if it otherwise decodes", async () => {
    ...
    await expect(processImageForStorage(svg)).rejects.toBeInstanceOf(InvalidImageError);
  });
});
