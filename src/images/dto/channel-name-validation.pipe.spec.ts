...
import { ChannelNameValidationPipe, MAX_CHANNEL_NAME_LENGTH } from "./channel-name-validation.pipe";
...
describe("ChannelNameValidationPipe", () => {
  ...
  it("rejects a value over the max length", () => {
    expect(() => pipe.transform("a".repeat(MAX_CHANNEL_NAME_LENGTH + 1))).toThrow(
      BadRequestException,
    );
  });
