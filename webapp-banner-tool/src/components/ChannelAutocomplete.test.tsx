...
function renderAutocomplete(
  overrides: Partial<React.ComponentProps<typeof ChannelAutocomplete>> = {},
) {
...
it("reopens after Escape and closes on Tab blur with correct combobox state", () => {
  renderAutocomplete();
  const input = screen.getByRole("combobox");
  fireEvent.focus(input);
  ...
  fireEvent.keyDown(input, { key: "ArrowDown" });
  expect(input).toHaveAttribute("aria-expanded", "true");
  expect(input).toHaveAttribute("aria-activedescendant", "channel-option-0");
