@echo off
title ts-icon Dev Startup (npm)
cd /d "%~dp0"

echo ============================================
echo   ts-icon - Dev Startup mit npm
echo ============================================
echo.

echo [1/4] Backend Dependencies installieren...
call npm install
if %errorlevel% neq 0 (echo FEHLER bei npm install & pause & exit /b 1)
...
call npm install
if %errorlevel% neq 0 (echo FEHLER bei Frontend npm install & pause & exit /b 1)
cd ..
...
start "ts-icon Public (3000)" cmd /k "cd /d %~dp0 && npm run start:public"

echo Starte Local Backend   (Port 3001)...
start "ts-icon Local (3001)" cmd /k "cd /d %~dp0 && npm run start:local"

echo Starte Frontend         (Port 5173)...
start "ts-icon Frontend (5173)" cmd /k "cd /d %~dp0\webapp-banner-tool && npm run dev"
